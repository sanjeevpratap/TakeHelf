...
class TweetTestCase(TestCase):
    def setUp(self):
        ...
        self.user=User.objects.create_user(username='cfe',password='pass2')
        self.user2=User.objects.create_user(username='cfe2',password='pass2')
        Tweet.objects.create(content="my first tweet",user=self.user)
        Tweet.objects.create(content="my first2 tweet",user=self.user)
       # Tweet.objects.create(content="my first3 tweet",user=self.user)
        Tweet.objects.create(content="my first3 tweet",user=self.user2)
    ...
    def get_client(self):
        client = APIClient()
        client.login(username=self.user.username, password='pass2')
        return client
